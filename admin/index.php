...
<body class="layout-boxed">
    <style>
        /* Personaliza el color del texto del botón de confirmación */
        .swal2-confirm {
            color: #FF2D00;
        ...
        /* Estilo para el elemento que mostrará el cursor pointer */
        .elemento-con-pointer {
            cursor: pointer;
        }
    </style>
    <!-- BEGIN LOADER -->
                        ...
                        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                            <div class="widget-content widget-content-area br-8">
                                ...
                                function acondicionarLink($link)
                                {
                                    ...
                                    $link = str_replace("/var/www/agropecuaria_usr11/data/www/agropecuariaojodeagua.com/admin/", "", $link);
                                ...
                                <div class="table-responsive">
                                    <div style="display: flex; justify-content: center; padding: 1rem;">
                                        <p>Total a pagar</p>
                                        <h3>$<?php echo number_format($adminFacturas->sumaFacturasEstaSemana(), 2)  ?></h3>
                                    </div>
                                    <div style="display: flex; justify-content: center; padding: 1rem;">
                                    <a href="api/generaReporteFacturas.php">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="35px" height="35px" viewBox="0 0 100 100" version="1.1">
                                           

                                            <path style="fill:#ffffff;stroke:#179B03;stroke-width:3;" d="m 68,3 0,19 19,0 z" />
                                            <path style="fill:#EBEBDA;stroke:#179B03;stroke-width:3;" d="m 68,3 0,19 19,0 0,74 -72,0 0,-93 53,0 19,19" />
                                    ...
                                    <table id="facturas" class="table dt-table-hover dataTable no-footer" style="width: 100%;" role="grid" aria-describedby="invoice-list_info">
                                        <thead>
                                            <tr role="row">
                                                ...
                                                <th class="sorting" tabindex="0" aria-controls="invoice-list" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending" style="width: 23px;">Moneda</th>
                                        ...
                                        <tbody>
                                            ...
                                            foreach ($facturas as $factura) {
                                                ...
                                                if($factura->moneda == 'USD'){
                                                    $cadenaMoneda = '<p style="color: green;">(USD $' .  number_format($factura->total_con_iva, 2) . ')</p> MNX $ ' . number_format(($factura->total_con_iva  * 17), 2)  . " APROX";
                                                }
                                                else{
                                                    $cadenaMoneda = 'MNX $' .  number_format($factura->total_con_iva, 2);
                                                }
                                            ?>
                                                ...
                                                <tr role="row">
                                                    ...
                                                    <td><?php echo $cadenaMoneda ?> </td>
                                                    <td><?php echo ($factura->moneda) ?> </td>
    ...
    <script>
        $('#facturas').DataTable({
            ...
            "dom": "<'dt--top-section'<'row'<'col-12 col-sm-4 d-flex justify-content-sm-start justify-content-center'l><'col-12 col-sm-4 d-flex justify-content-sm-start justify-content-center'><'col-12 col-sm-4 d-flex justify-content-sm-end justify-content-center mt-sm-0 mt-3'f>>>" +
                "<'table-responsive'tr>" +
    ...
    <script>
        function eliminar(id) {
            Swal.fire({
                title: "<?php echo '¿Desea eliminar esta factura?' ?>",
                showDenyButton: true,
                confirmButtonText: "<?php echo 'Sí' ?>",
                denyButtonColor: "#63E1F5",
    ...
    <script>
        function formatearFecha(fecha) {
            ...
            fecha = fecha[2] + " de " + fecha[1] + " del " + fecha[0];
            return fecha;
        }
    </script>
    ...
    <script>
        function aplazar(id) {
            var dias = document.getElementById('dias' + id).value;
            Swal.fire({
                title: "<?php echo '¿' ?>Desea aplazar el pago de la factura a la fecha " + formatearFecha(dias) + "?",
                showDenyButton: true,
                confirmButtonText: "<?php echo 'Sí' ?>",
                ...
                if (result.isConfirmed) {
                    console.log(id);
                    var datosPago = new FormData();
                    datosPago.append('accion', 'aplazar');
                    datosPago.append('id', id);
                    datosPago.append('dias', dias);
                    fetch("api/apiFacturas.php", {
                            method: "POST",
                            body: datosPago,
                        })
